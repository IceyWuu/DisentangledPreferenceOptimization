...
case "$LOSS_TYPE_UPPER" in
  DPO|SIMPO|LSIF|KTO|BCE|IPO|CPO|SLIC|UKL|DDRO|TIDPO) ;;
  *)
    echo "[ERROR] Unsupported LOSS_TYPE='$LOSS_TYPE'. Expected DPO, SIMPO, LSIF, BCE, IPO, CPO, SLIC, UKL, DDRO, TIDPO or KTO." >&2
    exit 1
    ;;
esac
...
if [[ -n "${DB_EMA_BETA}" ]]; then
  EXTRA_ARGS+=(db_ema_beta="${DB_EMA_BETA}")
fi

SAFE_LR="${LEARNING_RATE//./p}"
SAFE_LR="${SAFE_LR//-/_}"
DEFAULT_HUB_ID="Pythia-2B-Base-${LOSS_TYPE_UPPER}-${LEARNING_RATE}-temp"
...
echo "       DB_EMA_BETA     : ${DB_EMA_BETA:-<default from DPOConfig>}"
